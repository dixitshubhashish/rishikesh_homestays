...
- `404.html` — Custom error page (full site header/footer, quirky Rishikesh-themed copy, prominent phone/WhatsApp/email contact block, auto-redirects to `/` after 20s with a cancel option). Vercel and Netlify serve it automatically for any unmatched route (native `404.html`-at-root convention, no `vercel.json`/`_redirects` config needed); `server.js` has an explicit catch-all at the end mirroring that for local dev.
